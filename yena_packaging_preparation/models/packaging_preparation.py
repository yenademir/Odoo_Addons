...
class PackagingDeliveryReportXlsx(models.AbstractModel):
    ...
    def generate_xlsx_report(self, workbook, data, batches):
        ...
        headers = ['Purchase Order No', 'Drawing Number', 'Quantity', 'Is Unique?','Number of Euro-Pallets','Total Quantity']
        column_widths = [20, 30, 15, 15,20,15]
        ...
        for batch in batches:
            ...
            for prep in batch.packaging_preparation_ids:
                ...
                pallet_counts[pallet_no] = pallet_counts.get(pallet_no, 0) + 1
        ...
        for key in sorted_keys:
            ...
            for prep in preparations:
                ...
                pallet_unique_no = 0 if pallet_counts[pallet_no] == 1 else 1
                ...
                if blanket_order_line_exists:
                    ...
                    sheet.write(row, 7, prep.pallet_no, centered_format)
                    row += 1
                else:
                    sheet.write(row, 2, prep.customer_reference or '', centered_format)
                    ...
                    sheet.write(row,5 ,  pallet_unique_no,centered_format)
        ...
        if blanket_order_line_exists:
            sheet.write(row, 6, 'Total Pallets:', merge_format)
            sheet.write(row, 7, len(sorted_keys), merge_format)
    ...
    def generate_xlsx_report(self, workbook, data, batches):
        ...
        for batch in batches:
            ...
            for index,receiveFrom in enumerate(batch.vendor_ids,start=1):
                ...
                address_lines.append(f"{receive_address}")
            ...
            for index,customer in enumerate(batch.customer_ids,start=1):
                ...
                address_lines.append(f"{delivery_address}")
            ...
            row_height_boxes=len(customer_pallet_counts)*2-1
            
            sheet.merge_range(row, 0, row+row_height_boxes, 1 , "Kap Adedi ve Ebadı", merge_format12)
            sheet.merge_range(row, 2, row+row_height_boxes, 5, final_text, merge_format10)
            row+=row_height_boxes+1     
            
            sheet.merge_range(row, 0, row+2, 1, "AMBALAJLANMA", merge_format12)
            sheet.merge_range(row, 2, row+2, 5, total_pallet_count, merge_format10)
            row+=3      
...
class PackagingProposalFormReportXlsx(models.AbstractModel):
    ...
    def generate_xlsx_report(self, workbook, data, batches):
        ...
        for batch in batches:
            ...
            for line in batch.packaging_preparation_ids:
                product_key = f"{line.product_id}_{line.width}_{line.length}_{line.height}_{line.total_gross_weight}_{line.total_net_weight}_{line.stackable}"
                ...
                for transfer in matching_transfers:
                    if transfer.partner_id:
                        ...
                        if product_key not in product_info_by_vendor[address_key]['products']:
                            product_info_by_vendor[address_key]['products'][product_key] = {
                                'volume': line.width * line.length * line.height,
                                'pallet_no': line.pallet_no,
                                'width': line.width,
                                'length': line.length,
            ...
            for address_key, vendor_info in product_info_by_vendor.items():
                ...
                for product_key, product in vendor_info['products'].items():
                    ...
                    stackable_text = "İstiflenebilir" if product['stackable'] else "İstiflenemez"
                    volume_text = f"{product['width']}x{product['length']}x{product['height']}mm"
                    pallet_info = f"1 Kap, {volume_text}, {stackable_text}"
                    if pallet_info not in pallet_infos:
                ...
                sheet.merge_range(row, 2, row, 8, str(total_net_weight) + " kg", merge_format10)
                row += 1
                sheet.merge_range(row, 0, row, 1, "Toplam Brüt KG", merge_format12)
                sheet.merge_range(row, 2, row, 8, str(total_gross_weight) + " kg", merge_format10)
                row += 1
                sheet.merge_range(row, 0, row, 1, "Toplam Kap Adedi ", merge_format12)
                sheet.merge_range(row, 2, row, 8, str(total_pallet), merge_format10)
                row += 1
                
                all_pallet_infos = "\n".join(pallet_infos) 
                all_pallet_infos_len=len(pallet_infos)*2-2
